...
def delete_alpha_channel(path):
    for filename in tqdm(glob(os.path.join(path, '*.tiff'))):
        ...
        if data.shape[-1] == 4:
            ...
            tiff.imwrite(output_path, cleansed_data)
            print(f'Image {name} was successfully cleansed of its alpha channel')


# delete_alpha_channel('/mnt/lustre-grete/usr/u12649/scratch/data/melanoma_dataset/extracted/01_training_dataset_tif_ROIs')
...
check_for_empty_tiff('/mnt/lustre-grete/usr/u12649/scratch/data/puma/loaded_dataset/complete_dataset')