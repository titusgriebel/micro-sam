...
def get_cpm_paths(
    ...
    """
    if download is True:
        raise ValueError('Automated download not implemented')
    if data_choice == "cpm15":
        data_dir = os.path.join(path, data_choice)
        raw_dir, label_dir = "Images", "Labels"
        label_mat_paths = natsorted(glob(os.path.join(data_dir, label_dir, "*.mat")))
        raw_paths = natsorted(glob(os.path.join(data_dir, raw_dir, "*.png")))

    else:
        label_mat_paths = []
        raw_paths = []
        for split in ['train', 'test']:
            for label_mat in natsorted(glob(os.path.join(path, split, 'Labels', '*.mat'))):
                label_mat_paths.append(label_mat)
            for image in natsorted(glob(os.path.join(path, split, 'Images', '*.png'))):
                raw_paths.append(image)
        #print(len(raw_paths), len(label_mat_paths))
    # final raw_paths and label_paths creation after preprocessing
    label_paths = []
    ...
    #         for image in natsorted(glob(os.path.join(path, split, 'Images', '*.png'))):
    #             raw_paths.append(image)
    #         # for label in natsorted(glob(os.path.join(path, split, 'Labels', '*.tif'))):
    #         #     label_paths.append(label)
    assert len(raw_paths) == len(label_paths), "Inconsistency between raw and label data"
    ...
    print('Len of raw and label paths: ', len(label_paths))
